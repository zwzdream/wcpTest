...
public class FarmDocIndexManagerImpl implements FarmDocIndexInter {
	...
	public DataResult search(String word, String userid, Integer pagenum) throws Exception {
		...
		if (word.indexOf("FILECONTENT:") >= 0 && iql == null) {
			word = word.substring(word.indexOf("FILECONTENT:") + 12).replaceAll(":", "");
			iql = "WHERE(FILECONTENT=" + word + ")";
		}
		if (iql == null) {
			// word.substring(word.indexOf("TYPE:"));
			iql = "WHERE(TITLE,TEXT,TAGKEY,FILECONTENT,TYPENAME,AUTHOR=" + word.replaceAll(":", "") + ")";
		}
		if (pagenum == null) {
			pagenum = 1;
		}
		...
		for (Map<String, Object> node : result.getResultList()) {
			...
			String fileContent = node.get("FILECONTENT").toString();
			node.put("DOCDESCRIBE", text.length() > 256 ? text.substring(0, 256) : text);
			fileContent = fileContent.length() > 256 ? fileContent.substring(0, 256) : fileContent;
			node.put("fileNull",StringUtils.isEmpty(fileContent.trim())?"Y":"N");
			node.put("fileContent",fileContent);
