...
public class WebFileController extends WebUtils {
	...
	public ModelAndView editCommit(String docid, String fileId, String knowtype, String knowtitle, String knowtag,
		...
		try {
			...
			for (String fileid : fileids) {//循环得到fileid
				FarmDocfile file = farmFileManagerImpl.getFile(fileid);
				content = LuceneDocUtil.readFile(file.getFile(),file.getName());
				fileContent +=content;					
			}
			doc = wcpWebFileManagerImpl.editWebFile(fileContent,docid, Arrays.asList(fileId.trim().split(",")), knowtype, knowtitle,
	...
	public ModelAndView creatWebFileSubmit(String fileId, String creattype, String knowtype, String knowtitle,String fcontent,
		...
		try {
			...
			if (creattype != null && creattype.equals("on")) {
				...
				for (String fileid : fileids) {
					...
					content = LuceneDocUtil.readFile(file.getFile(),file.getName());
					fileContent +=content;					
					doc = wcpWebFileManagerImpl.creatWebFile(fileContent,fileidlist, knowtype, fileName, knowtag, docgroup, text,
				...
				doc = wcpWebFileManagerImpl.creatWebFile(fileContent,fileids, knowtype, knowtitle, knowtag, docgroup,  text,
						POP_TYPE.getEnum(writetype), POP_TYPE.getEnum(readtype), getCurrentUser(session));
				if (doc.getAudit() != null) {
